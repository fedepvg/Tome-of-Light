...
ATomeOfLightCharacter::ATomeOfLightCharacter()
{
...
void ATomeOfLightCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAction("ResetVR", IE_Pressed, this, &ATomeOfLightCharacter::OnResetVR);

	// Bind movement events
	PlayerInputComponent->BindAxis("MoveForward", this, &ATomeOfLightCharacter::MoveForward);
	PlayerInputComponent->BindAxis("MoveRight", this, &ATomeOfLightCharacter::MoveRight);

	// We have 2 versions of the rotation bindings to handle different kinds of devices differently
	// "turn" handles devices that provide an absolute delta, such as a mouse.
	// "turnrate" is for devices that we choose to treat as a rate of change, such as an analog joystick
	PlayerInputComponent->BindAxis("Turn", this, &APawn::AddControllerYawInput);
	PlayerInputComponent->BindAxis("TurnRate", this, &ATomeOfLightCharacter::TurnAtRate);
	...
	if (ProjectileClass != NULL)
	{
		...
		if (World != NULL)
		{
			if (bUsingMotionControllers)
			{
				...
				World->SpawnActor<ATomeOfLightProjectile>(ProjectileClass, SpawnLocation, SpawnRotation);
			}
			else
			{
				...
				World->SpawnActor<ATomeOfLightProjectile>(ProjectileClass, SpawnLocation, SpawnRotation, ActorSpawnParams);
			}
		}
	}
...
void ATomeOfLightCharacter::FireRomanFireball()
{
	if (RomanFireballClass != NULL)
	{
		UWorld* const World = GetWorld();
		if (World != NULL)
		{
...
void ATomeOfLightCharacter::BeginTouch(const ETouchIndex::Type FingerIndex, const FVector Location)
{
...
void ATomeOfLightCharacter::EndTouch(const ETouchIndex::Type FingerIndex, const FVector Location)
{
...
//void ATomeOfLightCharacter::TouchUpdate(const ETouchIndex::Type FingerIndex, const FVector Location)
//{
...
void ATomeOfLightCharacter::MoveForward(float Value)
{
	if (Value != 0.0f)
	{
...
void ATomeOfLightCharacter::MoveRight(float Value)
{
	if (Value != 0.0f)
	{
		// add movement in that direction
...
void ATomeOfLightCharacter::TurnAtRate(float Rate)
{
	// calculate delta for this frame from the rate information
	AddControllerYawInput(Rate * BaseTurnRate * GetWorld()->GetDeltaSeconds());
}

void ATomeOfLightCharacter::LookUpAtRate(float Rate)
{
	// calculate delta for this frame from the rate information
	AddControllerPitchInput(Rate * BaseLookUpRate * GetWorld()->GetDeltaSeconds());
}

bool ATomeOfLightCharacter::EnableTouchscreenMovement(class UInputComponent* PlayerInputComponent)
{
	if (FPlatformMisc::SupportsTouchInput() || GetDefault<UInputSettings>()->bUseMouseForTouch)
	{
		PlayerInputComponent->BindTouch(EInputEvent::IE_Pressed, this, &ATomeOfLightCharacter::BeginTouch);
		PlayerInputComponent->BindTouch(EInputEvent::IE_Released, this, &ATomeOfLightCharacter::EndTouch);

		//Commenting this out to be more consistent with FPS BP template.
		//PlayerInputComponent->BindTouch(EInputEvent::IE_Repeat, this, &ATomeOfLightCharacter::TouchUpdate);
		return true;
	}
